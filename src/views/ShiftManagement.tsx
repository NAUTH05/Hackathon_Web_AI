...
export default function ShiftManagement() {
  ...
  const [confirmDeleteShift, setConfirmDeleteShift] = useState<Shift | null>(
    null,
  );
  ...
  async function handleDeleteShift(shift: Shift) {
    try {
      ...
      showToast("success", "Đã xóa", `Ca "${shift.name}" đã được xóa.`);
      await loadData();
    } catch (err: unknown) {
      setConfirmDeleteShift(null);
      showToast("error", "Không thể xóa ca", (err as Error).message);
    }
  }
  ...
  return (
    <div>
                      ...
                      {!shiftForm.isOvernight &&
                        shiftForm.endTime < shiftForm.startTime && (
                          <p className="text-xs text-red-500 mt-1">
                            Giờ kết thúc phải ≥ giờ bắt đầu (hoặc bật &quot;Ca
                            qua đêm&quot;)
                          </p>
                        )}
                    </div>
                      ...
                      <div className="flex items-center gap-2">
                        {COLORS.map((c) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setShiftForm({ ...shiftForm, color: c })
                            }
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-200 flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500">
              {empTotal.toLocaleString()} nhân viên
            </span>
          </div>
          <Pagination
            page={empPage}
            ...
            total={empTotal}
            onPageChange={(p) => loadEmployees(p, empSearch)}
            label="nhân viên"
          />
      ...
      <ConfirmDialog
        ...
        confirmLabel="Xóa"
        onConfirm={() =>
          confirmDeleteShift && handleDeleteShift(confirmDeleteShift)
        }
