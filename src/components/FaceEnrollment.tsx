...
export function FaceEnrollment({ employeeId, onSuccess }: FaceEnrollmentProps) {
  ...
  const [showExistingFace, setShowExistingFace] = useState(false);

  useEffect(() => {
    async function checkExisting() {
      try {
        const emp = (await employeesApi.get(employeeId)) as {
        ...
        if (emp.faceImage) {
          setExistingFaceImage(emp.faceImage);
        }
      } catch (e) {
  ...
  useEffect(() => {
    loadModels()
      ...
      .catch((e) => {
        setResult({
          status: "error",
          message: "Lỗi tải mô hình AI. Vui lòng tải lại trang.",
        });
      });
    return () => {
      stopCamera();
  ...
  // When stream changes (after cameraActive renders the <video> element), attach it
  useEffect(() => {
  ...
  async function startCamera() {
    ...
    try {
      ...
      setStream(s);
      // After React re-renders and video element mounts, useEffect will attach stream
    } catch (err: any) {
  ...
  async function handleScanFace() {
    ...
    setResult({
      status: "capturing",
      message: "Đang phân tích khuôn mặt. Giữ nguyên vị trí...",
    });
    ...
    try {
      ...
      if (!detection) {
        setResult({
          status: "error",
          message:
            "Không tìm thấy khuôn mặt rõ nét. Vui lòng đảm bảo đủ sáng và nhìn thẳng.",
        });
        setDetecting(false);
        return;
      }
      ...
      setResult({
        status: "success",
        message:
          "Đăng ký khuôn mặt thành công! Bạn đã có thể dùng khuôn mặt này để chấm công.",
        snapshot,
      });
      setHasExistingDescriptor(true);
      setExistingFaceImage(snapshot);
      stopCamera();
      ...
      setResult({
        status: "error",
        message:
          (err as any).message || "Lỗi không xác định khi lưu khuôn mặt.",
      });
    } finally {
      setDetecting(false);
    }
      ...
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2 bg-gradient-to-r from-gray-50 to-white">
        ...
        <div className="ml-auto flex items-center gap-2">
          {hasExistingDescriptor && (
            <span className="text-[11px] bg-green-100 text-green-700 px-2.5 py-1 rounded-full flex items-center gap-1 font-bold border border-green-200">
              <CheckCircle2 className="w-3.5 h-3.5" /> Đã đăng ký
            </span>
          )}
          <button
            type="button"
            onClick={() => setShowExistingFace(true)}
            className="text-[11px] bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full flex items-center gap-1 font-medium border border-blue-200 hover:bg-blue-100 transition-colors cursor-pointer"
            title="Xem FaceID đã đăng ký"
          >
            <Eye className="w-3.5 h-3.5" /> Xem FaceID
          </button>
        </div>
      </div>

      {/* Modal xem FaceID đã đăng ký */}
      {showExistingFace && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              ...
              <button
                type="button"
                onClick={() => setShowExistingFace(false)}
            ...
            <div className="p-5">
              {existingFaceImage ? (
                <div className="space-y-3">
                  <div className="p-1.5 bg-gray-50 border border-gray-200 rounded-xl shadow-inner">
                    <img
                  ...
                  <p className="text-xs text-center text-green-600 font-medium flex items-center justify-center gap-1">
                    ...
                    Khuôn mặt đã được đăng ký trong hệ thống
                  </p>
                </div>
              ) : (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Nhân viên chưa đăng ký khuôn mặt trong hệ thống.
                  </p>
                </div>
              )}
            </div>
                ...
                {!cameraActive ? (
                  <div className="text-center p-6 text-gray-300">
                    ...
                    <p className="text-sm mb-4">
                      Camera đang tắt. Cần bật camera để quét khuôn mặt.
                    </p>
                    <button
                      type="button"
                      onClick={startCamera}
